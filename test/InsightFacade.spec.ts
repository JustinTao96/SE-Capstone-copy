...
describe("InsightFacade Add/Remove Dataset", function () {
    ...
    // Should reject a dataset with when the id parameter is already in the dataset
    it("Should reject a dataset with duplicate id", async () => {
        const id: string = "courses";
        const successCode: number = 204;
        const failCode: number = 400;
        let successResponse: InsightResponse;

        // Adding first dataset into the collection
        try {
            successResponse = await insightFacade.addDataset(id, datasets[id], InsightDatasetKind.Courses);
            let failResponse: InsightResponse;
            // Adding second dataset into the collection
            try {
                failResponse = await insightFacade.addDataset(id, datasets[id], InsightDatasetKind.Courses);
            } catch (err) {
                failResponse = err;
            } finally {
                expect(failResponse.code).to.equal(failCode);
            }
        } catch (err) {
            successResponse = err;
        } finally {
            expect(successResponse.code).to.equal(successCode);
        }
    });
    ...
    it("Should fail to remove courses dataset after it is already removed", async () => {
        ...
        try {
            ...
            expect(response.code).to.equal(successCode);
        }
    });
...
// should add a dataset at the start so that we can try to add/remove it
