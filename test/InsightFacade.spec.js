...
describe("InsightFacade Add/Remove Dataset", function () {
    ...
    it("Should reject a dataset with duplicate id", () => __awaiter(this, void 0, void 0, function* () {
        const id = "courses";
        const successCode = 204;
        const failCode = 400;
        let successResponse;
        try {
            successResponse = yield insightFacade.addDataset(id, datasets[id], IInsightFacade_1.InsightDatasetKind.Courses);
            let failResponse;
            try {
                failResponse = yield insightFacade.addDataset(id, datasets[id], IInsightFacade_1.InsightDatasetKind.Courses);
            }
            catch (err) {
                failResponse = err;
            }
            finally {
                chai_1.expect(failResponse.code).to.equal(failCode);
            }
        }
        catch (err) {
            successResponse = err;
            ...
            chai_1.expect(successResponse.code).to.equal(successCode);
        }
    }));
    ...
    it("Should fail to remove courses dataset after it is already removed", () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            chai_1.expect(response.code).to.equal(successCode);
        }
    }));
